...
def apply_overlay_to_buffer(buffer_segments, config, device_name, target_codec=None):
    """
    Re-encodes a list of video segments to add a text overlay.
    This is CPU-intensive, but only ever runs on the SHORT pre-event buffer -
    never on the main event recording.

    When an overlay is configured the pre-event buffer has to be re-encoded
    (you can't burn text into a stream copy). We re-encode it to the SAME codec
    as the main recording (passed in as target_codec, detected via ffprobe) so
    the two halves stay codec-compatible and can be concatenated with a stream
    copy - leaving the main recording completely untouched.
    ...
    # Match the (short) pre-event buffer to the MAIN recording's codec so the
    # later stream-copy concat works WITHOUT re-encoding the main event file.
    encoder, encode_opts = encoder_for_codec(target_codec)
    print(f"{device_name}: Encoding pre-event overlay as '{encoder}' to match main codec '{target_codec}'.")
    ...
    for i, segment_path in enumerate(buffer_segments):
        ...
        try:
            ffmpeg_command = [
                'ffmpeg',
                '-i', segment_path,
                # Re-encode video to apply the filter, using the SAME codec as
                # the main recording (detected via ffprobe) instead of a fixed
                # libx264, so the pre-roll stays splice-compatible with it.
                '-c:v', encoder,
                *encode_opts,
...
def get_video_codec(video_path):
    """
    Returns the codec name of the FIRST video stream (e.g. 'h264', 'hevc'),
    or None on failure. Uses ffprobe.

    This is what lets the pre-event buffer match the MAIN recording. The main
    event file is captured with '-c:v copy' (whatever the camera produces) and
    must NEVER be re-encoded. So rather than forcing everything to libx264, we
    probe the main file's codec here and re-encode only the (short) pre-event
    buffer to that same codec when an overlay forces a buffer re-encode. That
    keeps both halves codec-compatible for a stream-copy concat.
    """
    try:
        ...
        # For MPEG-TS, ffprobe lists the same video stream TWICE (once under the
        # program/PMT and once as a bare stream), so even with '-select_streams
        # v:0' the output can be e.g. "hevc\nhevc". .strip() only trims the ends
        # and would leave the embedded newline, breaking the codec->encoder
        # lookup. Take just the first non-empty line.
        lines = [ln.strip() for ln in result.stdout.splitlines() if ln.strip()]
        return lines[0] if lines else None
    except Exception as e:
        print(f"❌ FAILED to probe video codec. Error: {e}")
        return None

def encoder_for_codec(codec_name):
    """
    Maps a probed codec name to the software encoder that produces a
    concat-compatible stream of the SAME codec. Falls back to libx264, which
    preserves the script's previous behaviour if the codec is unknown or the
    probe failed.

    Returns (encoder_name, [extra_encode_options]). The extra options carry the
    -preset/-crf quality knobs only for encoders that accept them (x264/x265);
    other encoders get a sensible -q:v instead.
    """
    ...
    encoder = mapping.get(codec, 'libx264')

    if encoder in ('libx264', 'libx265'):
        # veryfast / crf 23: the same quality knobs the overlay path used before.
        # -pix_fmt yuv420p matches typical camera output and keeps the encoded
        # pre-roll bit-compatible with the (copied) main stream for concat.
        extra = ['-preset', 'veryfast', '-crf', '23', '-pix_fmt', 'yuv420p']
    else:
        extra = ['-q:v', '5']

    return encoder, extra

def color_range_full_flag(video_path):
    """
    Probe a file's video colour range and return the flag to stamp into the
    stream: '1' for full/JPEG range, '0' for limited/TV range. Defaults to '1'.

    Used so the final file has ONE consistent colour range. A prepended
    pre-event buffer (often tagged limited/yuv420p) and the event recording
    (often full/yuvj420p) otherwise flip pixel format at the splice, which forces
    the decoder's filter graph to reinitialise mid-stream - and that breaks
    hardware (VAAPI/QSV/NVENC) transcoding of the finished clip downstream.
    """
    try:
        result = subprocess.run(
        ...
        vals = [ln.strip().lower() for ln in result.stdout.splitlines() if ln.strip()]
        text = " ".join(vals)
        if 'pc' in vals or 'jpeg' in text or 'yuvj' in text:
            return '1'   # full range
        if 'tv' in vals or 'mpeg' in text:
            return '0'   # limited range
    except Exception as e:
        print(f"⚠️ Could not probe colour range for {video_path}: {e}")
    return '1'

def range_metadata_bsf(codec_name, full_flag):
    """
    Return the ffmpeg bitstream filter that stamps a uniform colour-range flag
    for this codec WITHOUT re-encoding the video. Returns None for codecs we
    don't handle (the caller then simply skips the filter).
    """
    codec = (codec_name or '').lower()
    if codec in ('hevc', 'h265'):
        return f'hevc_metadata=video_full_range_flag={full_flag}'
    if codec == 'h264':
        return f'h264_metadata=video_full_range_flag={full_flag}'
    return None
...
def record_video(device_name, ip_address, config, TrigType):
    ...
    try:
        ...
        # Decide the colour-range flag to stamp on the final file, taken from the
        # EVENT recording (the master content) BEFORE concat removes it. A
        # prepended pre-event buffer can be tagged with a different colour range;
        # stamping the whole stream with the event's range removes the mid-file
        # pixel-format switch that otherwise breaks GPU transcoding of the clip.
        repack_codec     = get_video_codec(temp_event_ts)
        repack_full_flag = color_range_full_flag(temp_event_ts)
        repack_vbsf      = range_metadata_bsf(repack_codec, repack_full_flag)
        ...
        if is_buffer_ready and snapshot_segments and os.path.exists(temp_event_ts):
            ...
            try:
                ...
                # --- B. Detect the MAIN recording's video codec, then overlay ---
                # The main event file is the master copy and must NEVER be
                # re-encoded (that would burn CPU and throw away the camera's
                # original encoding - the very thing we want to preserve). We
                # probe its codec and, if an overlay forces a re-encode of the
                # pre-event buffer, encode that buffer to the SAME codec. This is
                # the reverse of the old behaviour, which re-encoded the entire
                # event file to match the overlaid buffer.
                event_codec = get_video_codec(temp_event_ts)
                print(f"{device_name}: Main recording video codec detected as '{event_codec}'.")

                segment_files_for_concat = apply_overlay_to_buffer(
                    segment_files, config, device_name, event_codec
                )

                # The event file is ALWAYS concatenated as-is (stream copy).
                # No re-encode of the main recording, ever.
        ...
        try:
            ...
            if repack_vbsf:
                # Normalise the colour-range flag across the WHOLE stream without
                # re-encoding, so a prepended pre-event buffer and the event don't
                # switch pixel format mid-file. That switch forces a filter-graph
                # reinit that VAAPI/hardware transcoding can't survive. See
                # range_metadata_bsf / color_range_full_flag above.
                ffmpeg_command += ['-bsf:v', repack_vbsf]
            ffmpeg_command += [
                '-af', 'aresample=48000,acompressor=threshold=-21dB:ratio=4:makeup=2,loudnorm=I=-16:LRA=7:tp=-1.5',
                '-c:a', 'aac',
                '-b:a', '48k',
