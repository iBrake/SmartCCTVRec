...
def resume_rolling_buffer(device_name, buffer_proc):
    """
    Clears the camera's RAM buffer and un-pauses (SIGCONT) its rolling-buffer
    FFmpeg process, so pre-event capture starts fresh for the next event.

    record_video pauses the buffer (SIGSTOP) at the top; this is the matching
    "un-pause". It is safe to call no matter how record_video exits - including
    the NERF early-return path - so the buffer never gets stranded paused.
    """
    try:
...
def record_video(device_name, ip_address, config, TrigType):
    ...
    try:
        ...
        if not rapid_succession_detected:
            ...
            # A NERF still paused the rolling buffer at the top of this function,
            # so resume it before bailing out - otherwise pre-event capture would
            # stay frozen for every future event on this camera.
            resume_rolling_buffer(device_name, buffer_proc)
            return
        ...
        # Done the moment we've finished consuming the frozen pre-event buffer
        # (the prepend above was the last step that needed it) so the rolling
        # buffer restarts straight away, rather than waiting on the slow MP4
        # repackaging below.
        resume_rolling_buffer(device_name, buffer_proc)
